...
class _BrowserScreenState extends State<BrowserScreen> {
  ...
  /// Cached manga start page HTML (loaded once from assets).
  String? _homeHtml;
  ...
  void _handleWebScroll(int index, int x, int y) {
    ...
    // Keep visible while the address bar is focused.
    if (_urlFocusNode.hasFocus) {
      _lastScrollY = y;
      return;
    }
    final dy = y - _lastScrollY;
    if (dy > _scrollHideThreshold && _chromeVisible) {
      setState(() => _chromeVisible = false);
    } else if (dy < -_scrollHideThreshold && !_chromeVisible) {
      setState(() => _chromeVisible = true);
    }
    _lastScrollY = y;
  }
  ...
  Future<void> _closeTab(int index) async {
    final settings = context.read<BrowserSettings>();
    if (_tabs.length == 1) {
      final controller = _tabs[0].controller;
      setState(() {
        _tabs[0] = BrowserTab(
          id: _tabs[0].id,
          url: 'about:ink',
  ...
  Future<void> _ensureHomeHtml() async {
    if (_homeHtml != null && _homeHtml!.isNotEmpty) return;
    try {
      _homeHtml = await rootBundle.loadString('assets/start/home.html');
    } catch (e) {
      debugPrint('Failed to load home.html: $e');
      _homeHtml =
          '<!DOCTYPE html><html><head><meta charset="utf-8">'
          '<meta name="viewport" content="width=device-width,initial-scale=1">'
          '<title>Ink</title>'
          '<style>body{font-family:sans-serif;background:#F6F5F0;color:#121212;'
          'display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0}'
          '.box{border:4px solid #121212;padding:24px;box-shadow:6px 6px 0 #121212;text-align:center}'
          'h1{margin:0 0 12px;letter-spacing:2px}input,button{font-size:16px;padding:10px;border:3px solid #121212}'
          'button{background:#E60012;color:#F6F5F0;font-weight:800;margin-left:6px}</style></head>'
          '<body><div class="box"><h1>INK</h1>'
          '<form action="https://searx.be/search" method="get">'
          '<input name="q" placeholder="Search..." autofocus>'
          '<button type="submit">GO</button></form></div></body></html>';
    }
  }

  String _patchedHomeHtml() {
    final html = _homeHtml ?? '';
    final settings = context.read<BrowserSettings>();
    final raw = settings.searxngUrl.trim();
    String action;
    if (raw.contains('?')) {
      action = raw.split('?').first;
    } else if (raw.contains('/search')) {
      action = raw.replaceAll(RegExp(r'/search.*'), '/search');
    } else {
      action = raw.endsWith('/') ? '${raw}search' : '$raw/search';
    }
    if (action.isEmpty) action = 'https://searx.be/search';
    var patched = html.replaceFirst(
      'action="https://searx.be/search"',
      'action="$action"',
    );
    if (patched == html) {
      patched = html.replaceFirst(
        RegExp(r'action="[^"]*"'),
        'action="$action"',
      );
    }
  ...
  /// Loads the custom manga start page into [controller].
  Future<void> _loadInkHome(InAppWebViewController controller) async {
    await _ensureHomeHtml();
    final patched = _patchedHomeHtml();
    try {
      await controller.loadData(
        data: patched,
        mimeType: 'text/html',
        encoding: 'utf-8',
        baseUrl: WebUri('https://ink.local/'),
      );
    } catch (e) {
      debugPrint('loadData home failed: $e');
    }
    if (!mounted) return;
    setState(() {
  ...
  Future<void> _handleSubmitted(String input) async {
    ...
    if (trimmed.isEmpty ||
      ...
      if (controller != null) {
        ...
        setState(() => _currentTab.url = 'about:ink');
      }
...
""";
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: PreferredSize(
          preferredSize: Size.fromHeight(_chromeVisible ? appBarH : 0),
          child: AnimatedSlide(
  ...
  Widget _buildWebViewForTab(int index) {
    ...
    return KeyedSubtree(
      key: ValueKey(tab.id),
      child: InAppWebView(
        // Embed the real home HTML so the first frame is never a blank page.
        initialData: (tab.url == 'about:ink' ||
                tab.url == 'about:home' ||
                tab.url == 'ink://home')
            ? InAppWebViewInitialData(
                data: _homeHtml != null
                    ? _patchedHomeHtml()
                    : '<html><body style="background:#F6F5F0"></body></html>',
                mimeType: 'text/html',
                encoding: 'utf8',
                baseUrl: WebUri('https://ink.local/'),
              )
            : null,
        initialUrlRequest: (tab.url == 'about:ink' ||
                tab.url == 'about:home' ||
                tab.url == 'ink://home')
            ? null
